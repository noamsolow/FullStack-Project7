...
export function CampusRouteMap({
  ...
  const markerId = `campus-route-arrow-${useId().replaceAll(":", "")}`;
  ...
  return (
    <section className="maintenance-route-map" aria-label={title}>
      <div className="maintenance-route-map__heading">
        ...
        <div className="maintenance-route-map__legend" aria-label="Map legend">
          ...
          <span><i className="is-stop" /> {itemNoun} stop</span>
        </div>
      </div>

      <div className="maintenance-route-map__canvas">
        <img src="/campus-map.png" alt={imageAlt} />
        <svg
        ...
        >
          <defs>
            <marker id={markerId} markerWidth="2.8" markerHeight="2.8" refX="2.25" refY="1.4" orient="auto" markerUnits="userSpaceOnUse">
        ...
        {isPlottable(depot) && (
          <span
            ...
            aria-label={`${depotLabel}, building ${route.depotBuilding}`}
          >
            <Icon name="building" size={17} />
            <small>{depotLabel} {route.depotBuilding}</small>
          </span>
        )}
        ...
        {route.stops.map((stop, index) => {
          ...
          return (
            <button
              ...
              aria-label={`Stop ${index + 1}, building ${stop.campusCode}, ${items.length} ${itemNoun}s`}
        ...
        <div className="maintenance-route-map__details" aria-live="polite">
          <div className="maintenance-route-map__details-title">
            ...
            <div>
              ...
              <small>
                {selectedStop[itemsKey].length} {selectedStop[itemsKey].length === 1 ? itemNoun : `${itemNoun}s`} at this stop
              </small>
            </div>
          </div>
          <div className="maintenance-route-map__task-preview">
            {selectedStop[itemsKey].map(renderItem)}
          </div>
        </div>
      )}
    </section>
