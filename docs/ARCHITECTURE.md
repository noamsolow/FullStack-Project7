...
run SQL, repositories never produce HTTP responses, and route files contain no
business rules.
...
- Print PDFs are private to the owner, the selected print-center managers, and
  administrators.
- Stored filenames are UUIDs generated by the server; original names are
  metadata only and are never used as filesystem paths.
